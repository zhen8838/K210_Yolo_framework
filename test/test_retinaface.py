...
def test_generate_quan_img():
    ...
    h = RetinaFaceHelper('data/retinaface_img_ann.npy', [240, 320],
                         ...
                         [8, 16, 32], 0.35, [0.1, 0.2])
    i = 20
    d = Path('/home/zqh/workspace/retina_quan_img')
...
def test_ann_to_label():
    h = RetinaFaceHelper('data/retinaface_img_ann.npy', [640, 640],
                         [[[16, 16], [32, 32]], [[64, 64], [128, 128]], [[256, 256], [512, 512]]],
                         [8, 16, 32], 0.35, [0.1, 0.2])
    i = 3
...
def test_label_to_ann():
    ...
    re_label_landm = decode_landm(label_landm.numpy(), h.anchors.numpy(), h.variances.numpy())
    re_label_bbox = decode_bbox(label_loc.numpy(), h.anchors.numpy(), h.variances.numpy())
...
def test_tf_label_to_ann_compare():
    ...
    for number in range(10):
        with np.load(f"/home/zqh/workspace/Pytorch_Retinaface/loss_match_{number}.npz",
        ...
        for i in range(len(targets)):
            ...
            label_loc, label_landm, label_conf = h.ann_to_label(bbox, landm, clses, h.in_hw)
...
def test_data_process_pipe_line():
    ...
    @tf.function
    def _wrapper(i: tf.Tensor) -> tf.Tensor:
        path, ann = tf.numpy_function(
            lambda idx: tuple(image_ann_list[idx]),
            [i], [tf.string, tf.float32])
        img = h.read_img(path)
        ann = tf.split(ann, [4, 10, 1], 1)
...
def dev_loss_compare():
    ...
    number = 4
    for number in range(10):
        with np.load(f'/home/zqh/workspace/Pytorch_Retinaface/loss_test_{number}.npz',
                     allow_pickle=True) as d:
        ...
        for ann in targets:
            # NOTE 这里对比之前需要把ann_to_label里面的除img_hw给注释掉.
            for l, item in zip(ll, h.ann_to_label(*tf.split(ann, [4, 10, 1], 1), in_hw=h.in_hw)):
                l.append(item)
        loc_t, landm_t, conf_t = list(map(lambda l: np.stack(l), ll))

        true_y_true = np.concatenate([true_loc_t, true_landm_t, true_conf_t[:, :, None]], -1)

        y_true = tf.convert_to_tensor(np.concatenate([loc_t, landm_t, conf_t], -1), tf.float32)

        """ 先对比label是否相同 """
        assert np.allclose(y_true.numpy(), true_y_true.astype(np.float32))

        y_pred = tf.convert_to_tensor(np.concatenate([loc_data, landm_data, conf_data], -1), tf.float32)

        loss_loc, loss_landm, loss_conf = call(y_true, y_pred)
        loss = loss_loc + loss_landm + loss_conf
        try:
            """ 有时候会有0.1的差异。排查中 """
            assert np.allclose(loss.numpy(), true_loss_l + true_loss_c + true_loss_landm)
...
def test_face_detector_1mb_anchor():
    ...
    steps = [8, 16, 32, 64]
    in_hw = [300, 300]
    feature_maps = [[np.ceil(in_hw[0] / step).astype(int), np.ceil(in_hw[1] / step).astype(int)] for step in steps]
    ...
    ture_anchor = np.load('/home/zqh/workspace/Face-Detector-1MB-with-landmark/anchors.npy')
...
def test_retinafacenet_240_300_infer():
    ...
    infer_model.load_weights('log/240_320_retinaface_slim_exp/infer_model_238.h5', by_name=True)
...
def test_retinafacenet():
    infermodel, train_model = retinafacenet([640, 640, 3], 2, 64)
    train_model.outputs
    infermodel.outputs
...
def test_retinafacenet_k210():
    ...
    print(Path('tflite/retinaface_k210_train.tflite').stat().st_size / 1024, 'KB')  # 700KB
...
def test_retinafacenet_k210_v1():
    ...
    input_shape = [240, 320, 3]
    infer_model, train_model = retinafacenet_k210_v1(input_shape, [[[13, 17], [28, 38]], [[74, 98], [119, 161]], [[171, 217], [234, 299]]])
    ...
    print(Path('tflite/retinaface_k210_v1_final.tflite').stat().st_size / 1024, 'KB')
...
def test_retinafacenet_k210_v2():
    ...
    print(Path('tflite/retinaface_k210_train_v2.tflite').stat().st_size / 1024, 'KB')
...
def test_retinafacenet_k210_v3():
    ...
    print(Path('tflite/retinaface_k210_train_v3.tflite').stat().st_size / 1024, 'KB')  # 1957.015625 kb
...
def test_retinafacenet_slim():
    ...
    print(Path('tflite/retinaface_slim.tflite').stat().st_size / 1024, 'KB')  # 1340.0234375 kb
...
def test_find_anchor():
    ...
    xywhbbox = np.hstack([(bbox[:, 2:] - bbox[:, :2]) / 2, bbox[:, 2:] - bbox[:, :2]])
