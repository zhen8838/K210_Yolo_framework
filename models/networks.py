...
def pfld(input_shape: list, landmark_num: int, alpha=1.,
  ...
  conv1 = pipe(
      inputs, *[
          kl.Conv2D(int(64 * alpha), [3, 3], 2, **conv_kwargs),
  ...
  conv7 = pipe(
      conv6_1, *[
          kl.Conv2D(int(32 * alpha), 3, 2, **conv_kwargs),
          kl.BatchNormalization(),
          kl.ReLU(6)
      ])
  ...
  conv8 = pipe(
      conv7, *[
          kl.Conv2D(int(128 * alpha), 7, 1, **conv_kwargs),
          kl.BatchNormalization(),
          kl.ReLU(6)
      ])
...
def tiny_yolo(input_shape, anchor_num, class_num) -> [k.Model, k.Model]:
  ...
  x2 = compose(
      ...
      DarknetConv2D_BN_Leaky(1024, (3, 3)), DarknetConv2D_BN_Leaky(256, (1, 1)))(
          x1)
...
def yolo(input_shape, anchor_num, class_num) -> [k.Model, k.Model]:
  ...
  yolo_model_warpper = k.Model(
      inputs=inputs, outputs=[y1_reshape, y2_reshape, y3_reshape])
  ...
  for i in range(len(yolo_weight)):
    minshape = [
        min(new_weights[i].shape[j], yolo_weight[i].shape[j])
        for j in range(len(yolo_weight[i].shape))
    ]
...
def mbv2_ctdet(input_shape: list,
               class_num: int,
               filter_num: list,
               kernel_num: list,
               alpha: float = 1.0):
  input_tensor = k.Input(input_shape)
  body_model = MobileNetV2(
      include_top=False,
  ...
  out = DeconvLayer(len(filter_num), filter_num,
                    kernel_num)(body_model.output)  # shape=(?, 96, 96, 64)

  out_heatmap = pipe(
      out,
      *[kl.Conv2D(64, 3, 1, 'same'),
        kl.LeakyReLU(),
        kl.Conv2D(class_num, 1, 1)])

  out_wh = pipe(
      out, *[kl.Conv2D(64, 3, 1, 'same'),
             kl.LeakyReLU(),
             kl.Conv2D(2, 1, 1)])
  ...
  train_model = k.Model(input_tensor,
                        kl.Concatenate()([out_heatmap, out_wh, out_reg]))

  return infer_model, train_model


def shuffle_ctdet(input_shape: list,
                  class_num: int,
  ...
  out_8 = shufflenet_block(out_4, out_channel, 3, 2, shuffle_group=shuffle_group)
  for i in range(repeat - 1):
    out_8 = shufflenet_block(out_8, out_channel, 3, shuffle_group=shuffle_group)
  """ stage 16 """
  out_channel, repeat = channel_sizes[1]
  out_16 = shufflenet_block(
      out_8, out_channel, 3, 2, shuffle_group=shuffle_group)  # /16
  for i in range(repeat - 1):
    out_16 = shufflenet_block(out_16, out_channel, 3, shuffle_group=shuffle_group)
  """ stage 32 """
  out_channel, repeat = channel_sizes[2]
  # First block is downsampling
  out_32 = shufflenet_block(
      out_16, out_channel, 3, 2, shuffle_group=shuffle_group)  # /32
  for i in range(repeat - 1):
    out_32 = shufflenet_block(out_32, out_channel, 3, shuffle_group=shuffle_group)
  """ feature map fuse """
  ...
  """ detector """
  out_heatmap = compose(
      kl.Conv2D(feature_filters, 3, 1, 'same'), kl.ReLU(),
      kl.Conv2D(class_num, 1, 1, 'same'))(
          fuse3)
  out_wh = compose(
      kl.Conv2D(feature_filters, 3, 1, 'same'), kl.ReLU(),
      kl.Conv2D(2, 1, 1, 'same'))(
          fuse3)
...
def yolo_mbv1(input_shape: list, anchor_num: int, class_num: int,
  ...
  x = compose(
      ...
      kl.ReLU(6., name='block_20_relu6'), kl.UpSampling2D(2))(
          x)
  ...
  x = compose(
      ...
      kl.ReLU(6., name='block_24_relu6'), kl.UpSampling2D(2))(
          x)
...
def mbv1_imgnet(input_shape: list,
                class_num: int,
                depth_multiplier: float = 1.0,
                weights=None):
...
def mbv2_imgnet(input_shape: list,
                class_num: int,
                depth_multiplier: float = 1.0,
                weights=None):
...
def dcasetask5basemodel(input_shape,
                        nclasses,
                        softmax=False,
                        weight_decay=1e-4,
                        bn_decay=0.9,
