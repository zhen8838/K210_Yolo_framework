...
def Conv2D_BN_Leaky(*args, **kwargs):
  ...
  return compose(
      DarknetConv2D(*args, **no_bias_kwargs), kl.BatchNormalization(channel_axis),
...
def conv_bn(filters, strides=1):
  ...
  if strides == 2:
    return compose(kl.ZeroPadding2D(),
                   ...
                   kl.BatchNormalization(channel_axis), kl.ReLU())
  else:
    return compose(
...
def depth_conv2d(filters, kernel_size=1, strides=1, padding='valid'):
  ...
  if kernel_size == 3 and strides == 2 and padding == 'same':
    return compose(kl.ZeroPadding2D(),
                   kl.DepthwiseConv2D(kernel_size, strides, 'valid'), kl.ReLU(),
                   kl.Conv2D(filters, 1))
  else:
    return compose(
        kl.DepthwiseConv2D(kernel_size, strides, padding), kl.ReLU(),
        kl.Conv2D(filters, 1))
...
def retinaface_slim(input_shape: list, class_num: int,
  ...
  x14 = compose(
      kl.Conv2D(64, 1), kl.ReLU(), depth_conv2d(256, 3, 2, 'same'), kl.ReLU())(
          x13)

  loc_layers = [
      depth_conv2d(len(anchor[0]) * 4, 3, padding='same'),
  ...
  conf_layers = [
      depth_conv2d(len(anchor[0]) * class_num, 3, padding='same'),
  ...
  landm_layers = [
      depth_conv2d(len(anchor[0]) * 10, 3, padding='same'),
  ...
  infer_model = k.Model(inputs,
                        [bbox_regressions, ldm_regressions, classifications])
  train_model = k.Model(
      inputs,
      kl.Concatenate(-1)([bbox_regressions, ldm_regressions, classifications]))
...
def ullfd_slim(input_shape: list, class_num: int, anchor: List[Tuple]) -> k.Model:
  ...
  regressions = kl.Concatenate(1)(
      [kl.Reshape((-1, 4 + 1 + class_num))(regress) for regress in regressions])
...
def retinaface_rfb(input_shape: list, class_num: int,
                   anchor: List[Tuple]) -> k.Model:
  inputs = k.Input(input_shape)
