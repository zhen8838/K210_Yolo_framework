...
def main(config_file, new_cfg, mode, model, train):
    ...
    loop: BaseTrainingLoop = trainloop_register[train.trainloop](train_model, val_model)
    ...
    loop.set_summary_writer(tf.summary.create_file_writer(str(log_dir / datetime.strftime(datetime.now(), r'%Y%m%d-%H%M%S'))))
    ...
    loop.train_and_eval(epochs=train.epochs + initial_epoch,
