...
def main(data_dir: str, save_dir: str, output_file: str):
  ...
  imgrec: recordio = recordio.MXIndexedRecordIO(
      str(data_dir / 'train.idx'), str(data_dir / 'train.rec'), 'r')
  s = imgrec.read_idx(0)
  ...
  for i, ids in enumerate(tqdm(idmap, total=len(idmap))):
    ...
    for j, idx in enumerate(ids):
      ...
      tf.io.write_file(str(save_dir / fname), s)
      fnames.append(fname)
      labels.append(label)
    all_fnames.append(fnames)
    all_labels.append(labels)
  save_dict['train_data'] = {'fnames': all_fnames, 'lables': all_labels}
  save_dict['train_num'] = len(imgidx)

  print('Start make val and test set')
  target = 'lfw,cfp_fp,agedb_30'
  for name in target.split(','):
    with tf.io.TFRecordWriter(str(save_dir / f'{name}_val.tfrecords')) as writer:
      path = str(data_dir / (name+".bin"))
      data_list, issame_list = load_bin(path)
      print(f'Write {name} data into tfrecords\n')
      val_num = len(issame_list) * 2
      for img_src_list in data_list:
        for i in trange(len(issame_list)):
          label = issame_list[i]
          im_str_a = tf.image.encode_jpeg(
              img_src_list[i * 2], quality=100).numpy()
          im_str_b = tf.image.encode_jpeg(
              img_src_list[i*2 + 1], quality=100).numpy()
          serialized_example = make_example(im_str_a, im_str_b, label)
          writer.write(serialized_example)
    save_dict[f'{name}_val_data'] = str(save_dir / f'{name}_val.tfrecords')
    save_dict[f'{name}_val_num'] = val_num
