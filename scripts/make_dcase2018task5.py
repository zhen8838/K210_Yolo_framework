...
def write_to_tfrecord(res, writer, is_train):
  for melspec, label_int in tqdm(res, total=len(res)):
    ...
    # else:
    writer.write(make_example(tf.io.serialize_tensor(melspec).numpy(), label_int))
    # mel = np.random.choice(melspec, 1)
    # writer.write(make_example(tf.io.serialize_tensor(mel).numpy(), label_int))


def main():
  n = 1  # NOTE 为了方便起见，这里默认选分割1
  ratio = 0.1  # 默认选取10%的样本做有标签训练，剩余标签作为无标签数据
  seed = 10101
  NMEL = 40
  FRAME = 64  # 都是ms
  HOP = 20  # 都是ms
  ...
  for label in category.keys():
    ...
    choice_idx = np.random.choice(
        np.arange(len(idx)), int(len(idx) * ratio), replace=False)
  ...
  for name, ann_list in [('train_labeled', labeled_ann_list),
    ...
    for i in range(sup_task_num):
      res = pool.apply_async(
          worker_cgf,
          args=(ann_list[i * split:(i+1) * split], category, str(root_path), NMEL,
                FRAME, HOP),
    ...
    save_dict[name + '_num'] = len(ann_list) if is_train else len(ann_list)
  np.save('/home/zqh/Documents/K210_Yolo_framework/data/dcasetask5_ann_list.npy',
          save_dict)
