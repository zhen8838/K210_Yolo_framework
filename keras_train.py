...
from tools.custom import PFLDMetric, DummyMetric, DummyOnceMetric, SignalStopping
...
def main(config_file, new_cfg, mode, model, train, prune, quantize):
  ...
  if 'yolo' in model.name:
    ...
    losses = [
        loss_fn(h=h, layer=layer, name='loss', **model.loss_kwarg)
        for layer in range(
            len(train_model.output) if isinstance(train_model.output, list) else 1
        )
    ]  # type: List[YOLOAlignLoss]
    ...
    le = PFLDMetric(
        False,
        model.helper_kwarg['landmark_num'],
        train.batch_size,
        name='LE',
        dtype=tf.float32)
    fr = PFLDMetric(
    ...
    metrics = [
        DummyMetric(loss_obj.lwlrap, name='lwlrap'),
  ...
  for cbkparam in train.callbacks:
    ...
    if cbkparam.name == 'ModelCheckpoint':
      ...
      cbs.append(
          cbk_fn(val_model, validation_ds, vali_epoch_step, train_epoch_step - 1,
                 vali_acc_var, **cbkparam.kwarg))
    elif cbkparam.name == 'LwlrapValidation':
      cbs.append(
          cbk_fn(val_model, validation_ds, vali_epoch_step, train_epoch_step - 1,
                 val_lwlrap, **cbkparam.kwarg))
    else:
  ...
  if model.name in ['feacrec', 'semiaudio']:
    ...
    train_model.fit(
        train_ds,
        epochs=initial_epoch + train.epochs,
    ...
    train_model.fit(
        ...
        validation_steps=vali_epoch_step,
        verbose=train.verbose,
        initial_epoch=initial_epoch)
  """ Finish Training """
  ...
  if prune.is_prune == True:
    ...
    auto_saved_list = list(
        zip(auto_saved_list,
            [int(p.stem.split('_')[-1]) for p in auto_saved_list]))
