...
def yxyx_to_xywh(boxes: tf.Tensor) -> tf.Tensor:
...
def yxyx_to_xyxy(boxes: tf.Tensor) -> tf.Tensor:
    """Converts boxes from ymin, xmin, ymax, xmax to xmin, ymin, xmax, ymax.
...
def clip_keypoints(keypoints, image_shape):
    ...
    Args:
      keypoints: a tensor whose last dimension is 2 representing the coordinates
        of keypoints in `[y, x]` order.
      image_shape: a list of two integers, a two-element vector or a tensor such
        that all but the last dimensions are `broadcastable` to `keypoints`. The last
        dimension is 2, which represents [height, width].

    Returns:
      clipped_boxes: a tensor whose shape is the same as `keypoints` representing the
        clipped boxes.

    Raises:
      ValueError: If the last dimension of boxes is not 2.
    """
    ...
    with tf.name_scope('clip_boxes'):
        if isinstance(image_shape, list) or isinstance(image_shape, tuple):
            ...
            image_shape = tf.cast(image_shape, dtype=keypoints.dtype)
            height = image_shape[..., 0:1]
            width = image_shape[..., 1:2]
        ...
        clipped_y = tf.maximum(tf.minimum(y, height - 1.0), 0.0)
        clipped_x = tf.maximum(tf.minimum(x, width - 1.0), 0.0)

        clipped_boxes = tf.concat([clipped_y, clipped_x], axis=-1)
        return clipped_boxes
