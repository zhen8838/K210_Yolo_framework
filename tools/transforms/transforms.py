...
from tools.transforms.box_utils import clip_boxes, clip_keypoints
import math
...
def letterbox_resize(image: tf.Tensor, desired_size: tf.Tensor,
    ...
    def _resize(img, img_hw):
        ...
        img = tf.image.pad_to_bounding_box(img,
                                           tf.cast(offset[0], tf.int32),
    ...
    img, scale, offset = tf.cond(
        tf.reduce_all(tf.equal(size, img_hw)),
        lambda: (tf.cast(image, image.dtype if method == tf.image.ResizeMethod.NEAREST_NEIGHBOR else tf.float32),
                 tf.ones([2]), tf.zeros([2])),
...
def resize_clip_boxes(boxes: tf.Tensor,
...
def resize_keypoints(keypoints: tf.Tensor,
                     image_scale: tf.Tensor,
                     output_size: tf.Tensor,
                     offset: tf.Tensor) -> tf.Tensor:
...
def _check_bbox_area(min_y, min_x, max_y, max_x, image_height, image_width, delta=1):
    ...
    min_y, max_y = tf.cond(tf.equal(height, 0.0),
                           lambda: _adjust_bbox_boundaries(min_y, max_y, image_height),
                           lambda: (min_y, max_y))
    min_x, max_x = tf.cond(tf.equal(width, 0.0),
                           lambda: _adjust_bbox_boundaries(min_x, max_x, image_width),
                           lambda: (min_x, max_x))
    return min_y, min_x, max_y, max_x
...
def _rotate_bbox(bbox, image_height, image_width, degrees):
    ...
    min_y = -(tf.cast(tf.reduce_max(new_coords[0, :]), tf.float32) - 0.5 * image_height)
    min_x = tf.cast(tf.reduce_min(new_coords[1, :]), tf.float32) + 0.5 * image_width
    max_y = -(tf.cast(tf.reduce_min(new_coords[0, :]), tf.float32) - 0.5 * image_height)
    max_x = tf.cast(tf.reduce_max(new_coords[1, :]), tf.float32) + 0.5 * image_width

    # Clip the bboxes to be sure the fall between [0, 1].
    min_y, min_x, max_y, max_x = _clip_bbox(min_y, min_x, max_y, max_x, image_height, image_width)
    min_y, min_x, max_y, max_x = _check_bbox_area(min_y, min_x, max_y, max_x, image_height, image_width)
